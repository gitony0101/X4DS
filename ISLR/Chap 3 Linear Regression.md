...
### 1.4. P-value(Check $hypothesis~test$)
...
### 2.4. Assessing the Accuracy of the Model-By RSE and $R^2$
...
### 3.4. Notice：R2 & multiple regression & R2-adjusted in multiple regression
...
### 3.5. Summary for checking out a regression model
...
### 4.1. Remember – high p-value means NOT significant (p>0.15)

- specify 2 importance thresholds:
- p-to-add threshold (p≤ 0.15 to add)
- p-to-remove threshold (p>0.15 to remove)

### 4.2. Variables have an importance measure with threshold k
...
## 6. The danger of Stepwise Regression
